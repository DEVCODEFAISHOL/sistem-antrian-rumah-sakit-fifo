        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('staff.queues.update', $queue->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                         <div class="mb-4">
                            ...
                            <select name="patient_id" id="patient_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                ...
                                @foreach($patients as $patient)
                                    <option value="{{ $patient->id }}" {{ old('patient_id', $queue->patient_id) == $patient->id ? 'selected' : '' }}>{{ $patient->name }}</option>
                                @endforeach
                            </select>
                             @error('patient_id')
                           ...
                           <label for="poli_id" class="block text-gray-700 text-sm font-bold mb-2">Poli</label>
                             <select name="poli_id" id="poli_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                ...
                                @foreach($polis as $poli)
                                    <option value="{{ $poli->id }}" {{ old('poli_id', $queue->poli_id) == $poli->id ? 'selected' : '' }}>{{ $poli->nama }}</option>
                                @endforeach
                            </select>
                             @error('poli_id')
                                 <p class="text-red-500 text-xs italic">{{ $message }}</p>
                            ...
                            <select name="dokter_id" id="dokter_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                <option value="">-- Pilih Dokter --</option>
                                 @foreach($dokters as $dokter)
                                    <option value="{{ $dokter->id }}" {{ old('dokter_id', $queue->dokter_id) == $dokter->id ? 'selected' : '' }}>{{ $dokter->nama }}</option>
                                @endforeach
                            </select>
                            @error('dokter_id')
                                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="priority" id="priority" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                 <option value="ringan" {{ old('priority', $queue->priority) == 'ringan' ? 'selected' : '' }}>Ringan</option>
                                <option value="sedang" {{ old('priority', $queue->priority) == 'sedang' ? 'selected' : '' }}>Sedang</option>
                                 <option value="berat" {{ old('priority', $queue->priority) == 'berat' ? 'selected' : '' }}>Berat</option>
                            </select>
                             @error('priority')
                                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                             @enderror
                       ...
                       <div class="mb-4">
                            ...
                            <input type="time" name="appointment_time" id="appointment_time" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('appointment_time', $queue->appointment_time ? $queue->appointment_time->format('H:i') : null) }}">
                         ...
                         <div class="mb-4">
                           <label for="keterangan" class="block text-gray-700 text-sm font-bold mb-2">Keterangan (Opsional)</label>
                             <textarea name="keterangan" id="keterangan" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('keterangan', $queue->keterangan) }}</textarea>
                             @error('keterangan')
                                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                             @enderror
                        </div>
                         <div class="mb-4">
                            ...
                            <input type="date" name="checkup_date" id="checkup_date" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('checkup_date', $queue->checkup_date->format('Y-m-d')) }}" required>
                             @error('checkup_date')
                                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                             @enderror
                        </div>
                       <div class="mb-4">
                            ...
                            <select name="jenis_kunjungan" id="jenis_kunjungan" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                <option value="baru" {{ old('jenis_kunjungan', $queue->jenis_kunjungan) == 'baru' ? 'selected' : '' }}>Baru</option>
                                <option value="lama" {{ old('jenis_kunjungan', $queue->jenis_kunjungan) == 'lama' ? 'selected' : '' }}>Lama</option>
                            </select>
                        ...
                        <div class="mb-4">
                            <label for="complaint" class="block text-gray-700 text-sm font-bold mb-2">Keluhan</label>
                             <textarea name="complaint" id="complaint" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('complaint', $queue->complaint) }}</textarea>
                           @error('complaint')
                                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                             @enderror
                        </div>
                         <div class="mb-4">
                            ...
                            <input type="checkbox" name="is_emergency" id="is_emergency" class="form-checkbox h-5 w-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500" {{ old('is_emergency', $queue->is_emergency) ? 'checked' : '' }}>
                            @error('is_emergency')
                                 <p class="text-red-500 text-xs italic">{{ $message }}</p>
                            @enderror
                        </div>
