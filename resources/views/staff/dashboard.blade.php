<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Dashboard Staff - Poliklinik Khusus Penyakit Dalam Rumah Sakit Budhi Asih') }}
        </h2>
    </x-slot>
        ...
        <div class="grid grid-cols-1 gap-6 mb-8 md:grid-cols-3">
            ...
            <div class="p-6 bg-white rounded-lg shadow-md dark:bg-gray-700">
                <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200">Antrian Saat Ini (Seluruh Prioritas)</h3>
                 ...
                 @if ($currentQueue)
                      <form action="{{ route('staff.queues.call', $currentQueue->id) }}" method="POST" class="mt-4">
                            @csrf
                           <button type="submit"
                                class="px-4 py-2 text-white bg-blue-500 rounded-md hover:bg-blue-600">Panggil Antrian</button>
                            <form action="{{ route('staff.queues.skip', $currentQueue->id) }}" method="POST"
            ...
            <!-- Card Antrian Saat Ini (Ringan - Sedang) -->
             <div class="p-6 bg-white rounded-lg shadow-md dark:bg-gray-700">
                ...
                @if ($currentQueueLightMedium)
                     <form action="{{ route('staff.queues.call', $currentQueueLightMedium->id) }}" method="POST"
                        class="mt-4">
                        @csrf
                         <button type="submit"
                            class="px-4 py-2 text-white bg-green-500 rounded-md hover:bg-green-600">Panggil Antrian</button>
                         <form action="{{ route('staff.queues.skip', $currentQueueLightMedium->id) }}" method="POST"
                            class="inline-block ml-2">
                              @csrf
                             <button type="submit"
                                class="px-4 py-2 text-white bg-red-500 rounded-md hover:bg-red-600">Lewati</button>
               ...
               <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200">Antrian Saat Ini (Berat)</h3>
                <p class="text-2xl font-bold text-gray-800 dark:text-gray-200">
                 ...
                 @if ($currentQueueHeavy)
                      <form action="{{ route('staff.queues.call', $currentQueueHeavy->id) }}" method="POST"
                        class="mt-4">
                        @csrf
                           <button type="submit"
                                class="px-4 py-2 text-white bg-red-500 rounded-md hover:bg-red-600">Panggil Antrian</button>
                         <form action="{{ route('staff.queues.skip', $currentQueueHeavy->id) }}" method="POST"
                            class="inline-block ml-2">
                              @csrf
                              <button type="submit"
                                class="px-4 py-2 text-white bg-red-500 rounded-md hover:bg-red-600">Lewati</button>
                 ...
                 <table class="min-w-full">
                    <thead class="bg-gray-50 dark:bg-gray-600">
                     ...
                     <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-700 dark:divide-gray-600">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-200">{{ ucfirst($queue->status) }}</td>
                                 <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-200">
                                    @if ($queue->status === 'called')
                                        <form action="{{ route('staff.queues.complete', $queue->id) }}" method="POST"
                                           class="inline">
                                            @csrf
                                               <button type="submit"
                                                    class="px-2 py-1 text-white bg-green-500 rounded-md hover:bg-green-600">Selesai</button>
                                       </form>
                                       <form action="{{ route('staff.queues.skip', $queue->id) }}" method="POST"
                                          class="inline">
                                               @csrf
                                               <button type="submit"
                         ...
                         @empty
                            <tr>
                                  <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-200" colspan="8">Tidak ada data antrian hari ini.</td>
                             </tr>
                         @endforelse
                     </tbody>
                 </table>
