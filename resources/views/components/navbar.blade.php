<nav aria-label="secondary" x-data="{ open: false }"
    ...
    <div class="flex items-center gap-3">
        <!-- Toggle dark mode button (desktop) -->
        ...
        <x-dropdown align="right" width="48">
            ...
            <x-slot name="content">
                ...
                @if(Auth::user()->hasRole('admin'))
                    ...
                    <x-dropdown-link :href="route('admin.queues.index')">
                        {{ __('Kelola Antrian') }}
                    </x-dropdown-link>
                    <x-dropdown-link :href="route('admin.patients.index')">
                        {{ __('Kelola Pasien') }}
                    </x-dropdown-link>
                ...
                <!-- Tampilkan menu khusus staff -->
                @if(Auth::user()->hasRole('staff'))
                    <x-dropdown-link :href="route('staff.dashboard')">
                        {{ __('Staff Dashboard') }}
                    </x-dropdown-link>
                    <x-dropdown-link :href="route('staff.queues.index')">
                        {{ __('Kelola Antrian') }}
                    </x-dropdown-link>
                @endif
...
<div class="fixed inset-x-0 bottom-0 flex items-center justify-between px-4 py-4 sm:px-6 transition-transform duration-500 bg-white md:hidden dark:bg-dark-eval-1"
    ...
    @if (auth()->check() && auth()->user()->hasRole('admin'))
        ...
        @elseif(auth()->check() && auth()->user()->hasRole('staff'))
            <a href="{{ route('staff.dashboard') }}" class="inline-flex items-center gap-2">
            @else
                <a href="{{ route('/') }}" class="inline-flex items-center gap-2">
    @endif
